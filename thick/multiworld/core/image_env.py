...
class ImageEnv(ProxyEnv, MultitaskEnv):
    def __init__(
        ...
        if flatten_img:
            ...
            img_space = Box(0, 255, (self.imsize, self.imsize, 3), dtype=np.uint8)
        ...
        self.return_image_proprio = return_image_proprio
        if self.return_image_proprio and 'proprio_observation' in spaces.keys():
    ...
    def _get_flat_img(self):
        ...
        assert (self.flatten_img and image_obs.shape[0] == self.channels) or\
               (not self.flatten_img and image_obs.shape[-1] == self.channels)

        if self.flip_img:
            image_obs = np.flip(image_obs, axis=0)

        if not self.flatten_img:
            return image_obs
