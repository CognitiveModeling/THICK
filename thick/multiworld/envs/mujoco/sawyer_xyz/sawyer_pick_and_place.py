...
class SawyerPickAndPlaceEnv(MultitaskEnv, SawyerXYZEnv):
    def __init__(
        ...
        self.target = target
        if obstacle:
            obj_init_positions=((0, 0.55, 0.016),)
    ...
    def model_name(self):
        if self.obstacle:
            return get_asset_full_path('sawyer_xyz/sawyer_pick_and_place_obstacle.xml')
        elif self.target:
            return get_asset_full_path('sawyer_xyz/sawyer_pick_and_place_target.xml')
    ...
    def get_target_pos(self):
        if not self.obstacle and not self.random_init:
            return np.array([0.0, 0.67, 0.22])  # fixed target
        if self.random_init:
            return self.model.site_pos[self.model.site_name2id('target-site')].copy()
            #raise NotImplementedError("Only available in obstacle environment")
    ...
    def set_target_marker(self, goal):
        self.model.site_pos[self.model.site_name2id('target-site')] = (
            goal[:3]
        )
    ...
    def reset_model(self):
        ...
        if self.random_init:
            goal = np.random.uniform(
                self.hand_and_obj_space.high[3:] - np.array([0, 0.07, 0]), # TODO self.hand_and_obj_space.low[3:], # TODO
                self.hand_and_obj_space.high[3:] - np.array([0, 0.07, 0]), # TODO
        ...
        self.set_goal(self.sample_goal())
        self._set_goal_marker(self._state_goal)
        hand_goal = np.random.uniform(
                self.hand_and_obj_space.low[3:] + 0.05,
                self.hand_and_obj_space.high[3:] - 0.05,
                size=(1, self.hand_and_obj_space.low.size - 3),
        )
        if self.random_init:
            self.set_target_marker(np.array([0.0, 0.75, hand_goal[0, 2]]))
        self.picked_up_object = False
        return self._get_obs()
