...
class Driver:
  ...
  def __call__(self, policy, steps=0, episodes=0):
    ...
    while step < steps or episode < episodes:
      ...
      policy_outs, self._state = policy(obs, self._state, **self._kwargs) # here distinguish between actiomn and output
      #print("POLICY UTS =", policy_outs)
      actions = {'action': policy_outs['action']}
      actions = [
      ...
      for i, (act, ob) in enumerate(zip(actions, obs)):
        tran = {k: self._convert(v) for k, v in {**ob, **act}.items()}  # here add context and goal as well
