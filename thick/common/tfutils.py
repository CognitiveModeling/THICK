...
class Module(tf.Module):
  ...
  def load(self, filename):
    my_values = tf.nest.map_structure(lambda x: x.numpy(), self.variables)
    my_amount = len(tf.nest.flatten(my_values))
    print(f'Trying to fill {my_amount} tensors.')
    with pathlib.Path(filename).open('rb') as f:
      values = pickle.load(f)
    amount = len(tf.nest.flatten(values))
    count = int(sum(np.prod(x.shape) for x in tf.nest.flatten(values)))
    print(f'Load checkpoint with {amount} tensors and {count} parameters from {filename}')
...
class Optimizer(tf.Module):
  ...
  def reset(self):
    for var in self._opt.variables():
