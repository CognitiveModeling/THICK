...
class RandomAgent:
  ...
  def __call__(self, obs, state=None, mode=None):
    ...
    if self._env is not None:
      action = self._env.scripted_action()
    output = {'action': action, 'goal': None, 'context': None}
    if self.logprob:
...
def long_term_return(reward, value, pcont, inter_reward, inter_discount):
  assert reward.shape.ndims == value.shape.ndims, (reward.shape, value.shape)
  if isinstance(pcont, (int, float)):
    pcont = pcont * tf.ones_like(reward)

  expected_return_tau = reward + pcont * value  # expected r + gamma * v at tau
  return inter_reward + inter_discount*(expected_return_tau), expected_return_tau  # accounting for everything passed until tau
...
class StreamNorm(tfutils.Module):
  ...
  def get(self):
    metrics = {}
    outputs = self.transform(self.mag)
    metrics['normed_mean'] = outputs.mean()
    metrics['normed_std'] = outputs.std()
    return metrics

  def get_mean(self):
    return {'mean': self.mag.mean()}
