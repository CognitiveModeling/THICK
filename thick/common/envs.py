...
class GymWrapper:
  ...
  def __init__(self, env, obs_key='image', act_key='action', env_obs_key='image'):
    self._env = env
    self._obs_is_dict = hasattr(self._env.observation_space, 'spaces')
    self._act_is_dict = hasattr(self._env.action_space, 'spaces')
    self._obs_key = obs_key
    self._act_key = act_key
    self._special_obs_key = env_obs_key != 'image'
    self._env_obs_key = env_obs_key
  ...
  @property
  def obs_space(self):
    if self._obs_is_dict and not self._special_obs_key:
      spaces = self._env.observation_space.spaces.copy()
    elif self._special_obs_key:
      shapes = self._env.observation_space[self._env_obs_key]
      shapes.shape = (64, 64, 3)
      spaces = {self._obs_key: shapes}
    else:
      shapes = self._env.observation_space
      shapes.shape = (64,64,3)
      spaces = {self._obs_key: shapes}
    overall_obs_space = {
        **spaces,
  ...
  def step(self, action):
    ...
    if not self._obs_is_dict:
      obs = {self._obs_key: tf.image.resize(obs, (64, 64), method='nearest')}
    if self._special_obs_key:
      obs = {self._obs_key: tf.image.resize(obs[self._env_obs_key], (64, 64), method='nearest')}
  ...
  def reset(self):
    ...
    if self._special_obs_key:
      obs = {self._obs_key: tf.image.resize(obs[self._env_obs_key], (64, 64), method='nearest')}
    elif not self._obs_is_dict:
      obs = tf.image.resize(obs, (64, 64), method='nearest')
    if not self._obs_is_dict:
  ...
  def __init__(self, name, action_repeat=1, size=(64, 64), camera=None):
    ...
    if domain == 'manip':
      ...
      if domain == "manipulator":
        task += "_ball"  # bring_ball or insert_ball
...
class SuccessWrapper:
  ...
  def __init__(self, env, rew_succ=1, key='log_success'):
    self._env = env
    self._rew_succ = rew_succ
    self._key = key
  ...
  def obs_space(self):
    ...
    spaces[self._key] = gym.spaces.Box(0, 1, (), dtype=np.bool)
    return spaces

  def step(self, action):
    obs = self._env.step(action)
    obs[self._key] = obs['reward'] >= self._rew_succ
    return obs
