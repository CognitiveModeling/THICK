...
def main(argv):
  ...
  if prefill:
    ...
    random_train_env = None
    random_eval_env = None
    random_train_agent = common.RandomAgent(act_space, env=random_train_env)
    random_eval_agent = common.RandomAgent(act_space, env=random_eval_env)
    train_driver(random_train_agent, steps=prefill, episodes=1)
    eval_driver(random_eval_agent, episodes=int(config.eval_eps * 10)) # start with more than one to avoid empty set
    train_driver.reset()
    eval_driver.reset()
